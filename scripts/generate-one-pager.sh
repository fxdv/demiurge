...
lines = [
    ...
    "implemented and gated locally; Track B engineering path green (XDP veth, io_uring TCP serve, load scenarios);",
    "production exit gates (real NIC XDP, x86_64 p99) and fleet economics remain **Track B/C**.",
    "",
    "## Validation",
    "",
    f"| Check | Result |",
    ...
    f"| CPU bench gates | **{len(gate_rows)}/{len(gate_rows)}** recorded |",
...
lines += [
    ...
    "| Binary | Purpose |",
    "|--------|---------|",
    "| `demiurge-router` | Phase-aware TCP forwarder (env-configured; production entrypoint) |",
    "| `xtask` | `gen`, `lint`, `bench-gate`, `bench-probe`, `load-bench`, `load-report`, `ab-bench` |",
    "",
    "```bash",
    "# Example: local forwarder with two pools",
    "export DEMIURGE_LISTEN=127.0.0.1:8080",
    "export DEMIURGE_PREFILL='pf0@127.0.0.1:9001@0.01,pf1@127.0.0.1:9002@0.012'",
    "export DEMIURGE_DECODE='dc0@127.0.0.1:9101@0.02,dc1@127.0.0.1:9102@0.025'",
    "./bin/demiurge-router",
    "```",
    "",
    "## Artifact contents",
    ...
    "",
    "## Track B engineering (Linux)",
    "",
    "- Runtime XDP attach on veth + router kernel admit (`track-b-gate.sh`, `track-b-verify.sh`)",
    "- Router hardening: module split, bounded worker pool (`DEMIURGE_WORKER_THREADS`), connection cap (`DEMIURGE_MAX_CONNS`)",
    "- `IoUringProxySession` on production TCP `serve()`; `LOAD-TRACK-B-IOURING` / `LOAD-TRACK-B-KERNEL` load scenarios",
    "- `cargo xtask ab-bench` — routing policy A/B vs baselines",
    "- `docs/THREAT-MODEL.md` — wire-protocol security contract",
    "- `LOAD-TRACK-B-KERNEL` in Gate workflow Track B job + linux-nightly pre-release",
    "- `BENCH-IOURING-FWD` micro-bench + full load/stress on Linux VM",
    "",
    "## 'sim & verify",
    "",
    "- `./scripts/apostrophe-sim.sh` — trace-driven fleet replay (`SIM-FLEET-*`)",
    "- `./scripts/verify.sh full` — gate + load + stress + harden Tiers 1–4 + `'sim` (`SIM-FLEET-*`)",
    "- Topology ELI5: `design/topologies/demiurge-topologies-el5.svg`",
    "",
    "## Out of scope (Track B exit gates / Track C)",
    "",
    "- XDP shed on production NIC under fleet load",
    "- x86_64 reference hardware p99 budget under CP slowdown",
    "- RDMA KV hand-off, live migration (Track C)",
    "",
    "## Traceability",
    "",
